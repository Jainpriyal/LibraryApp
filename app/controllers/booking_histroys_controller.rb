class BookingHistroysController < ApplicationController
        ...
        if(myTime < Time.now)
          ...
          if(!booked)
            ...
            redirect_to book_room_url
                   ...
                   #toTime = myTime + 2.hours
                  #hashArg= {:from => myTime, :to => toTime, :date => myDate, :rooms_id => @room.id}
                  #@booking_histroy = BookingHistroy.new(hashArg)
                  #@booking_histroy.save
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to booking_histroys_url, notice: 'Booking histroy was successfully destroyed.' }
